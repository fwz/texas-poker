import { useEffect, useRef, useState } from 'react';
import type { PublicGameState } from '../../../shared/types';
import { getSocket } from '../hooks/useSocket';
import { playSound } from '../hooks/useSound';

interface Props {
  state: PublicGameState;
  playerId: string;
}

function clamp(v: number, min: number, max: number) {
  return Math.max(min, Math.min(max, v));
}

function getDefaultRaise(state: PublicGameState, meChips: number, meBet: number): number {
  const minRaiseTotal = state.currentBet + state.minRaise;
  const maxRaise = meChips + meBet;
  const pot = state.pot || state.blinds.big * 2;
  // When facing a raise above big blind, default to 2× current bet (larger than opponent's)
  const facingRaise = state.currentBet > state.blinds.big;
  const raw = facingRaise
    ? state.currentBet * 2
    : state.currentBet + Math.round(pot * 0.5);
  return clamp(raw, minRaiseTotal, maxRaise);
}

export function ActionBar({ state, playerId }: Props) {
  const isMyTurn = state.activePlayerId === playerId && state.phase !== 'showdown' && state.phase !== 'waiting';
  const me = state.players.find(p => p.id === playerId);

  const minRaiseTotal = state.currentBet + state.minRaise;
  const maxRaise = me ? me.chips + me.bet : minRaiseTotal;
  const defaultRaise = me ? getDefaultRaise(state, me.chips, me.bet) : minRaiseTotal;

  const [raiseAmount, setRaiseAmount] = useState(defaultRaise);
  const [inputVal, setInputVal] = useState(String(defaultRaise));
  const [showRaise, setShowRaise] = useState(false);
  const [selectedPreset, setSelectedPreset] = useState<string | null>(null);
  const [timeLeft, setTimeLeft] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);

  // Countdown timer
  useEffect(() => {
    if (!isMyTurn || !state.turnDeadline) return;
    const update = () => setTimeLeft(Math.max(0, Math.ceil((state.turnDeadline! - Date.now()) / 1000)));
    update();
    const id = setInterval(update, 200);
    return () => clearInterval(id);
  }, [isMyTurn, state.turnDeadline]);

  if (!isMyTurn || !me) return null;

  const canCheck = me.bet >= state.currentBet;
  const callAmount = Math.min(state.currentBet - me.bet, me.chips);
  const canRaise = me.chips > callAmount;

  const setAmount = (v: number, preset?: string) => {
    const clamped = clamp(v, minRaiseTotal, maxRaise);
    setRaiseAmount(clamped);
    setInputVal(String(clamped));
    setSelectedPreset(preset ?? null);
  };

  const pot = state.pot || state.blinds.big * 2;
  const presets = [
    { label: '33%', raw: state.currentBet + Math.round(pot * 0.33) },
    { label: '50%', raw: state.currentBet + Math.round(pot * 0.50) },
    { label: '66%', raw: state.currentBet + Math.round(pot * 0.66) },
    { label: '100%', raw: state.currentBet + pot },
    { label: '全押', raw: maxRaise },
  ].map(p => ({ ...p, amount: clamp(p.raw, minRaiseTotal, maxRaise) }));

  const send = (action: any, sound: Parameters<typeof playSound>[0]) => {
    playSound(sound);
    getSocket().emit('player_action', action, () => {});
  };

  const handleRaiseClick = () => {
    if (!showRaise) {
      setShowRaise(true);
      setTimeout(() => inputRef.current?.focus(), 50);
    } else {
      send({ type: 'raise', amount: raiseAmount }, 'raise');
    }
  };

  const handleDelayCard = () => {
    getSocket().emit('use_delay_card', () => {});
  };

  const btnBase = 'flex-1 rounded-2xl font-bold text-sm transition-all active:scale-95 min-h-[56px] flex flex-col items-center justify-center gap-0.5';
  const isUrgent = timeLeft > 0 && timeLeft <= 5;

  return (
    <div className="fixed bottom-0 left-0 right-0 bg-gray-900/95 backdrop-blur border-t border-gray-700 px-3 pt-2 pb-3">
      {/* Raise panel */}
      {showRaise && canRaise && (
        <>
          <div className="flex gap-2 mb-2">
            {presets.map(p => {
              const active = selectedPreset === p.label;
              return (
                <button
                  key={p.label}
                  onClick={() => setAmount(p.amount, p.label)}
                  className={`flex-1 py-1.5 rounded-xl text-xs font-semibold border transition-all min-h-[36px]
                    ${active
                      ? 'bg-green-700 border-green-400 text-white scale-105'
                      : 'bg-gray-800 border-gray-600 text-gray-300 active:bg-gray-700'}`}
                >
                  {p.label}
                </button>
              );
            })}
          </div>
          <div className="flex items-center gap-2 mb-2">
            <input
              type="range"
              min={minRaiseTotal}
              max={maxRaise}
              step={state.blinds.big}
              value={raiseAmount}
              onChange={e => setAmount(Number(e.target.value), undefined)}
              className="flex-1 accent-green-500"
            />
            <input
              ref={inputRef}
              type="number"
              value={inputVal}
              onChange={e => setInputVal(e.target.value)}
              onBlur={() => setAmount(Number(inputVal))}
              onFocus={e => e.target.select()}
              onKeyDown={e => e.key === 'Enter' && setAmount(Number(inputVal))}
              className="w-20 px-2 py-1.5 rounded-lg bg-gray-800 border border-yellow-600 text-yellow-300 text-center text-sm font-bold"
            />
          </div>
        </>
      )}

      {/* Timer + delay card row */}
      {state.turnDeadline !== undefined && (
        <div className="flex items-center justify-between mb-2 px-1">
          <div className={`font-mono font-bold text-lg tabular-nums ${isUrgent ? 'text-red-400 animate-pulse' : 'text-yellow-400'}`}>
            {timeLeft}s
          </div>
          <button
            onClick={handleDelayCard}
            disabled={me.delayCards <= 0}
            className="flex items-center gap-1 px-3 py-1 rounded-xl bg-gray-800 border border-gray-600 text-xs font-semibold text-gray-300 disabled:opacity-40 active:bg-gray-700"
          >
            <span>💳</span>
            <span>延时</span>
            <span className="text-yellow-400 font-bold">×{me.delayCards}</span>
          </button>
        </div>
      )}

      {/* Main action buttons */}
      <div className="flex gap-2">
        {!canCheck && (
          <button
            onClick={() => send({ type: 'fold' }, 'fold')}
            className={`${btnBase} bg-red-800 active:bg-red-950 text-white border border-red-700`}
            style={{ boxShadow: '0 2px 8px rgba(220,38,38,0.3)' }}
          >
            弃牌
          </button>
        )}

        {canCheck ? (
          <button
            onClick={() => send({ type: 'check' }, 'check')}
            className={`${btnBase} bg-gray-600 active:bg-gray-800 text-white border border-gray-500`}
            style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.3)' }}
          >
            过牌
          </button>
        ) : (
          <button
            onClick={() => send({ type: 'call' }, 'call')}
            className={`${btnBase} bg-blue-700 active:bg-blue-900 text-white border border-blue-600`}
            style={{ boxShadow: '0 2px 8px rgba(37,99,235,0.35)' }}
          >
            <span>跟注</span>
            <span className="text-yellow-300 text-xs font-normal">{callAmount}</span>
          </button>
        )}

        <button
          onClick={handleRaiseClick}
          disabled={!canRaise}
          className={`${btnBase} border disabled:opacity-40 transition-colors
            ${showRaise
              ? 'bg-green-600 active:bg-green-800 border-green-400 text-white ring-2 ring-green-400/50'
              : 'bg-green-700 active:bg-green-900 border-green-600 text-white'}`}
          style={{ boxShadow: '0 2px 8px rgba(22,163,74,0.35)' }}
        >
          <span>{showRaise ? '确认' : canCheck ? '下注' : '加注'}</span>
          {showRaise && <span className="text-yellow-300 text-xs font-normal">{raiseAmount}</span>}
        </button>
      </div>
    </div>
  );
}
